            ...
            if (router.query.signup) {
                createUserWithEmailAndPassword(auth, email, password)
                    .then((userCredential) => {
                        ...
                        setUser({ email, image: Profile.src, id: user.uid, name: 'User', password: user.reloadUserInfo.passwordHash })
                        login({ ...user, displayName: 'User', password: user.reloadUserInfo.passwordHash, providerData: [{ photoURL: Profile.src }] })
                    })
                    .catch((error) => {
        ...
        <>
            ...
            >
                <div className="flex  flex-col items-center justify-center   mx-auto md:h-screen lg:py-0">
                    {error && <div className="bg-red-500 px-4 py-2 text-white flex self-end absolute right-4 rounded top-2 z-10">{error}</div>}
                    <div className=" px-[48px]  py-[50px] absolute bg-[#FFFFFF] shadow-[0px_6px_8px_rgba(0,0,0,0.05)] rounded-[5px] ">
                        ...
                        <div className="flex items-center gap-x-2 my-[12px] bg-[#FFFFFF] border border-[#EAEBF6] rounded-full py-[12px] px-[24px]">
                            ...
                            <input placeholder="Type here" onClick={() => setError('')} onChange={e => setEmail(e.target.value)} value={email} className="text-[#C0C3D0] outline-none w-full px-4 py-2 border-none" />
                        ...
                        <div className="flex justify-between items-center gap-x-2 my-[12px] bg-[#FFFFFF] border border-[#EAEBF6] rounded-full py-[12px] px-[24px]">
                            ...
                            <input placeholder="******" onClick={() => setError('')} type={show ? "text" : "password"} onChange={e => setPassword(e.target.value)} value={password} className="text-[#C0C3D0] outline-none w-full px-4 py-2 border-none items-center" />
                            <div className="text-[#C0C3D0] cursor-pointer">
                                <Image src={show ? "/images/eye-slash.svg" : "/images/eye.svg"} width={24} height={24} alt="" onClick={() => setShow(!show)} />
                            </div>
                        </div>
                        <div onClick={signinWithEmail} className="flex text-[#FFFFFF] font-bold text-center justify-center items-center gap-x-2 my-[20px] bg-[#625BF7]  rounded-[10px] py-[12px] px-[24px] cursor-pointer">
                            CONTINUE
                        </div>
                        ...
                        <div onClick={signinWithGoogle} className="flex justify-center gap-x-2 px-[55px] py-[18px] items-center 
